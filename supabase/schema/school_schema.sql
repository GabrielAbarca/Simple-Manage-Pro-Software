...
-- `kind` says where a category's score comes from: 'assignments' is the
-- normal points ratio, 'attendance' is derived from the attendance record
-- (REAC 2026's 5% component). Names drift and are teacher-editable, so the
-- attendance component cannot be identified by its name.
...
create unique index grade_categories_one_attendance_per_cst
